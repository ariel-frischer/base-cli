...
func colorizedHelp(cmd *cobra.Command, args []string) {
...
func printColorizedFlags(flagUsages string) {
	for _, line := range strings.Split(strings.TrimRight(flagUsages, "\n"), "\n") {
		...
		// cobra formats flags as "  --flag type   description"
		// Find where the description starts (after the flag+type portion)
		trimmed := strings.TrimLeft(line, " ")
		indent := len(line) - len(trimmed)

		// Split at double-space boundary between flag portion and description
...
// splitFlagLine splits "--flag type   description" into flag and description parts.
func splitFlagLine(s string) []string {
	// Find consecutive spaces that separate the flag from its description.
	// Cobra uses variable-width padding, but there's always 3+ spaces.
	for i := 0; i < len(s)-2; i++ {
