...
# base-cli Installer
# Usage: curl -fsSL https://raw.githubusercontent.com/ariel-frischer/base-cli/main/install.sh | sh
#
# Environment variables:
#   BASE_CLI_INSTALL_DIR - Installation directory (default: ~/.local/bin)
#   BASE_CLI_VERSION     - Specific version to install (default: latest)

set -eu

# Configuration
GITHUB_REPO="ariel-frischer/base-cli"
BINARY_NAME="base-cli"
...
detect_os() {
    case "$(uname -s)" in
        ...
        MINGW*|MSYS*|CYGWIN*)
            ...
            printf 'base-cli requires WSL (Windows Subsystem for Linux).\n\n' >&2
...
main() {
    ...
    local version="${BASE_CLI_VERSION:-}"
    if [ -z "$version" ]; then
    ...
    local install_dir="${BASE_CLI_INSTALL_DIR:-$DEFAULT_INSTALL_DIR}"
    ...
    echo "Get started:"
    echo ""
    echo "    ${BINARY_NAME} init my-project"
